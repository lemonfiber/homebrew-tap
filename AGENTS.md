...
Homebrew formulae. The formula is **generated** by `cli`'s release CI — do not
edit `Formula/*.rb` by hand. A change to the generator is a `cli` change following
the normal lifecycle (`REPO-R28`). Spec:
...
formula, the change probably belongs in `cli`'s release pipeline instead.
